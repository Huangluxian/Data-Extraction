...
/*获取指定长度的 ...0001 字符串*/
string get_String_One(int length)
{
    string code;
...
/*块边界重构算法，通过与上下左右四块的边界对比，确定差异大小*/
int BAF(unsigned char CB[][8], unsigned char UP[][8], unsigned char RT[][8], unsigned char DW[][8], unsigned char LF[][8])
{
    int sum = 0;
    for (int i = 0; i < 8; i++) {
        sum += abs(CB[0][i] - UP[7][i]);
        sum += abs(CB[i][7] - RT[i][0]);
        sum += abs(CB[7][i] - DW[0][i]);
        sum += abs(CB[i][0] - LF[i][7]);
    }
    return sum;
}